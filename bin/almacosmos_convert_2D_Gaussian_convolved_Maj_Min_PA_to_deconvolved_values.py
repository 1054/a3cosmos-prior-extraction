...
if len(sys.argv) <= 6:
    print('Usage: ')
    print('    almacosmos_convert_2D_Gaussian_convolved_Maj_Min_PA_to_deconvolved_values.py Maj Min PA beam_Maj beam_Min beam_PA')
    print('    # Note that Maj, Min, beam_Maj are the convolved (appearent) source size FWHM in arcsec unit, and beam_Maj, beam_Min are beam FWHM in arcsec unit.')
    print('')
    sys.exit()
...
# 
Maj_out_convol = numpy.array(float(sys.argv[1]))
Min_out_convol = numpy.array(float(sys.argv[2]))
PA_out_convol  = numpy.array(float(sys.argv[3]))
...
# Print
# 
asciitable.write(numpy.column_stack((Maj_out, Min_out, PA_out)), sys.stdout, 
                 Writer=asciitable.FixedWidth, delimiter='|', bookend=True, 
                 names = ['Maj_out_deconvolved', 'Min_out_deconvolved', 'PA_out_deconvolved'])
