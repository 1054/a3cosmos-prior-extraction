...
column_x = '1' # column number starts from 1.
column_y = '2' # column number starts from 1.
...
data_xerr = []
data_yerr = []
data_x = data_table.getColumn(int(column_x)-1) if column_x.isdigit() else data_table.getColumn(column_x)
...
# 
# MCMC
MDL = pymc.MCMC(my_model(x1, y_obs))
MDL.sample(1e4)
pprint(MDL.stats())
