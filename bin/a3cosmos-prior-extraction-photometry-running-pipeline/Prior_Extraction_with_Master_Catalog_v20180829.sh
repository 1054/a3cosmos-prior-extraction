...
InputSci="../ALMA_Calibrated_Images_by_Magnelli/20180829/a3cosmos_meta_table_without_highres_within_cosmos.fits.files.txt"
...
for (( i=$istart; i<=$iend; i+=$istep )); do
    ...
    echo "Adding screen task \"$RunScreen\" with command \"a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian  final negative -unlock image_check getpix galfit gaussian sersic negative final -overwrite final 2>&1 | tee log_$RunScreen.txt\""
    screen -d -S "$RunScreen" -m bash -c "cd $RunFolder; a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian  final negative -unlock image_check getpix galfit gaussian sersic negative final -overwrite final 2>&1 | tee log_$RunScreen.txt"
