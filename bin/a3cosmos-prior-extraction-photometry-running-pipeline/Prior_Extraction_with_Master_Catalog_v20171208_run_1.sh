...
for (( i=$istart; i<=$iend; i+=$istep )); do
    ...
    echo "Adding screen task \"$RunScreen\" with command \"a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian sersic final negative -unlock image_check getpix galfit gaussian sersic negative final 2>&1 | tee log_$RunScreen.txt\""
    screen -d -S "$RunScreen" -m bash -c "cd $RunFolder; a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian sersic final negative -unlock image_check getpix galfit gaussian sersic negative final 2>&1 | tee log_$RunScreen.txt"
