...
# Set Output dir
# 
OutputDir="Prior_Extraction_with_Master_Catalog_v20170526_Shifted_Test_04" #<TODO>#
...
for (( i=0; i<${#SciImages[@]}; i++ )); do
    ...
    #      astrodepth_prior_extraction_photometry -cat  "$InputCat"  -sci  "$SciImage"  -psf  "$PsfImage"  -output-name "$SourceName"  -steps "negative"  -unlock "negative"  -overwrite "negative" ${UserOverwrite[@]}
           astrodepth_prior_extraction_photometry -cat  "$InputCat"  -sci  "$SciImage"  -psf  "$PsfImage"  -output-name "$SourceName"  -steps "getpix" "galfit" "gaussian" "final" "negative"  ${UserOverwrite[@]}
    #      astrodepth_prior_extraction_photometry -cat  "$InputCat"  -sci  "$SciImage"  -psf  "$PsfImage"  -output-name "$SourceName"  -steps "final"  -overwrite "final"
