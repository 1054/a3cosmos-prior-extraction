...
InputSci="../ALMA_Calibrated_Images_by_Magnelli/v20180102/fits_file_list_sorted_excluded_very_high_res.txt"
...
for (( i=$istart; i<=$iend; i+=$istep )); do
    ...
    while       [[   18  -le    $(screen -ls | grep "caap_" | wc -l) ]]; do
    sleep            24
    echo "Counting screen task "$(screen -ls | grep "caap_" | wc -l)
    done
    RunScreen="caap_$(date +%Y%m%d)_$(date +%s)_$i"
    echo "Adding screen task \"$RunScreen\" with command \"a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian  final negative -unlock image_check getpix galfit gaussian sersic negative final -overwrite negative final 2>&1 | tee log_$RunScreen.txt\""
    screen -d -S "$RunScreen" -m bash -c "cd $RunFolder; a3cosmos-prior-extraction-photometry -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian  final negative -unlock image_check getpix galfit gaussian sersic negative final -overwrite negative final 2>&1 | tee log_$RunScreen.txt"
