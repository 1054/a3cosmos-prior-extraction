...
InputSci="../ALMA_Calibrated_Images_by_Magnelli/v20170604/fits_file_list_sorted_v20170724_excluded_very_high-res.txt"
OutputDir="Prior_Extraction_with_Master_Catalog_v20171208" # $(echo "${BASH_SOURCE[0]}" | sed -e 's/\.sh//g' | sed -e 's%^./%%g')
...
for (( i=$istart; i<=$iend; i+=$istep )); do
    ...
    while       [[   10  -le    $(screen -ls | grep "caap_" | wc -l) ]]; do
    sleep            24
    echo "Counting screen task "$(screen -ls | grep "caap_" | wc -l)
    done
    RunScreen="caap_$(date +%s)_$i"
    echo "Adding screen task \"$RunScreen\" with command \"a3cosmos-prior-extraction-photometry-fix-bug-20171221 -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian sersic final negative -unlock image_check getpix galfit gaussian sersic negative final 2>&1 | tee log_$RunScreen.txt\""
    screen -d -S "$RunScreen" -m bash -c "cd $RunFolder; a3cosmos-prior-extraction-photometry-fix-bug-20171221 -start $i -end $(($i+$istep-1)) -out \"$OutputDir\" -step getpix galfit gaussian sersic final negative -unlock image_check getpix galfit gaussian sersic negative final 2>&1 | tee log_$RunScreen.txt"
